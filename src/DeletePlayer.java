...
public class DeletePlayer extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jLabel3.setText("Delete Player Data");
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        if (textField1.getText().compareTo("") != 0 && textField2.getText().compareTo("") != 0 && textField3.getText().compareTo("") != 0 && textField4.getText().compareTo("") != 0 && textField5.getText().compareTo("") != 0 && textField6.getText().compareTo("") != 0 && textField7.getText().compareTo("") != 0 && textField8.getText().compareTo("") != 0)
        {
            try
            {
                deleteItem = JOptionPane.showConfirmDialog(null, "Are you sure you want to delete player \"" + id + "\" from the database?", "Warning", JOptionPane.YES_NO_OPTION);
                if (deleteItem == JOptionPane.YES_OPTION)
                {
                    Class.forName("com.mysql.cj.jdbc.Driver");
                    sqlConn = DriverManager.getConnection(dataConn, username, password);
                    pst = sqlConn.prepareStatement("delete from nsms_play_br where PlayerCode=?");
                    pst.setString(1, id);
                    pst.executeUpdate();
                    JOptionPane.showMessageDialog(this, "Successfully deleted player data!");
                    updateDB();
                }//end if
            }//end try
            catch (Exception e)
